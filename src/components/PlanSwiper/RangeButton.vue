...
const selectedPriority = ref(planStore.taskRangeInfo);
const selectedPriorityIndex = ref<number>(planStore.taskRangeIndex);

planStore.$subscribe((_,state) => {
  selectedPriority.value = state.taskRangeInfo;
  selectedPriorityIndex.value = state.taskRangeIndex;
});
...
<template>
    <div class="priority-dropdown">
      <button @click="toggleDropdown" class="dropdown-button" :style="{ backgroundColor: buttonColor }">
        <span :class="selectedPriority.value.class">{{ selectedPriority.value.symbol }}</span>
      </button>
...
const isDropdownOpen = ref(false);
const selectedPriority = ref({label: "重要且紧急", value: 1, class: "priority-urgent-important", symbol: "Ⅰ" },);
